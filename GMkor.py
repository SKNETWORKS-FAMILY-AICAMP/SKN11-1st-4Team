...
csv_writer.writerow(['brand', 'question', 'answer'])
...
def crawl_page():
    ...
    for question_btn in question_btns:
        ...
        # 클릭하여 답변을 표시하도록 하기
        try:
            question_btn.click()  # 질문을 클릭하여 답변을 표시
            time.sleep(2)  # 답변이 로드될 시간을 기다림

            # 답변 영역 찾기
            answer = driver.find_element(By.CSS_SELECTOR, 'div.answer')
            print(f"답변: {answer.text}")

            # CSV 파일에 데이터 작성
            csv_writer.writerow(['chevrolet', question, answer.text])
            ...
            sql = """
                ...
                VALUES (%s, %s, %s)
            """
            values = ('chevrolet', question, answer.text)
