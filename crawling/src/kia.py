...
def crawl_page():
    ...
    for idx, question_btn in enumerate(question_btns):
        ...
        try:
            ...
            # 답변 영역 찾기 (수정된 부분)
            answer = driver.find_element(By.XPATH, f'//*[@class="container responsivegrid"]//*[@class="faqinner__wrap"]//p')
            print(f"답변: {answer.text}")

            # CSV 파일에 데이터 작성
            csv_writer.writerow(['kia', question, answer.text])
            ...
            sql = """
                ...
                VALUES (%s, %s, %s)
            """
            values = ('kia', question, answer.text)
